...
class DBHandler:
    ...
    def searchStudents(self,name):
        ...
        try:
            ...
            # Create cursor object
            cursor = db.cursor(pymysql.cursors.DictCursor)
            query = "Select id,name,rollno,semmester,cgpa  from students where name =%s"
            args=(name)
            ...
            for r in results:
                #studentsList.append(Student(r["name"],r["rollno"],r["semmester"],r["cgpa"]))
                record=[]
                record.append(r["name"])
                record.append(r["rollno"])
                record.append(r["semmester"])
                record.append(str(r["cgpa"]))
                studentsList.append(record)
