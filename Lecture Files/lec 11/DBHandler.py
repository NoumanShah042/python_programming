...
class DBHandler:
    ...
    def insertRecord(self, student):
        ...
        try:
            db = pymysql.connect(
                host=self.dbipaddress, user=self.dbuser, password=self.dbpwd, database=self.dbname)
            cursor = db.cursor(pymysql.cursors.DictCursor)
            query = "insert into students(rollno,name,semmester,cgpa) values(%s,%s,%s,%s)"
            args = (student.rollno, student.name,
                    student.semmester, student.cgpa)
    ...
    def fetchAllStudents(self):
        ...
        try:
            ...
            for r in results:
                studentsList.append(
                    Student(r["name"], r["rollno"], r["semmester"], r["cgpa"]))
...
std = Student('Huraira', 'bcsf18m006', 4, 4.0)
db.insertRecord(std)
